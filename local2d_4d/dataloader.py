...
class Loader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = np.zeros([self.max_frames, 4], dtype=np.float32)
        label[:, 3] = 1
        label[range(start_frame + 1, end_frame)] = [0, 0, 1, 0]
        if start_frame != end_frame:
            label[start_frame] = [0.5, 0, 0.5, 0]
            label[end_frame] = [0, 0.5, 0.5, 0]
        else:
            label[start_frame] = [0.333, 0.333, 0.333, 0]

        return frame_vecs, frame_mask, ques_vecs, ques_mask, label, start_frame, end_frame
